...
class BertWwm(nn.Layer):
    ...
    def get_vocab_path(self):
        ...
        """
        save_path = os.path.join(DATA_HOME, 'chinese-bert-wwm', 'bert-wwm-chinese-vocab.txt')
        if not os.path.exists(save_path) or not os.path.isfile(save_path):
            url = "http://paddlenlp.bj.bcebos.com/models/transformers/bert/bert-wwm-chinese-vocab.txt"
            download(url, os.path.join(DATA_HOME, 'chinese-bert-wwm'))
        return save_path
