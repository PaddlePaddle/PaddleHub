...
class ErnieV2(nn.Layer):
    ...
    def get_vocab_path(self):
        ...
        """
        save_path = os.path.join(DATA_HOME, 'ernie_v2_eng_large', 'vocab.txt')
        if not os.path.exists(save_path) or not os.path.isfile(save_path):
            url = "https://paddlenlp.bj.bcebos.com/models/transformers/ernie_v2_large/vocab.txt"
            download(url, os.path.join(DATA_HOME, 'ernie_v2_eng_large'))
        return save_path
