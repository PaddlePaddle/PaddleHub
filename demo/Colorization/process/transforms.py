...
from Colorization.process.functional import *
...
class ColorConvert:
    def __init__(self, mode='RGB2LAB'):
        self.mode = mode
...
class ColorizePreprocess:
    def __init__(self, ab_thresh=0., p=.125, num_points=None, samp='normal', use_avg=True, is_train = True):
    ...
    def __call__(self, data_lab):
        ...
        for nn in range(N):
            ...
            while cont_cond:
                if self.num_points is None:  # draw from geometric
                    # embed()
                    cont_cond = np.random.rand() < (1 - self.p)
                ...
                if self.use_avg:
                    # embed()
                    data['hint_B'][nn, :, h:h + P, w:w + P] = np.mean(
                        np.mean(data['B'][nn, :, h:h + P, w:w + P], axis=2, keepdims=True), axis=1, keepdims=True).reshape(
                        1, C, 1, 1)
                else:
        ...
        data['mask_B'] -= 0.5
        if self.is_train:
            data['A'] = np.squeeze(data['A'], 0).astype(np.float32)
            data['B'] = np.squeeze(data['B'], 0).astype(np.float32)
            data['real_B_enc'] = np.squeeze(data['real_B_enc'], 0).astype(np.int64)
            data['hint_B'] = np.squeeze(data['hint_B'], 0).astype(np.float32)
            data['mask_B'] = np.squeeze(data['mask_B'], 0).astype(np.float32)
        else:
...
class ColorPostprocess:
    def __init__(self, type=np.uint8):
        self.type = type
