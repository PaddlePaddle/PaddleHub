...
def train():
    ...
    train_reader = paddle.batch(paddle.reader.shuffle(reader,
                                                      buf_size=batch_size),
                                batch_size,
                                drop_last=True)
    ...
    features, logits = model.net(input=image,
                                 class_dim=reader_config.num_classes)
        ...
        if args.outdir is not None:
            try:
                ...
                fluid.io.save_params(executor=exe,
                                     dirname=args.outdir + '/' + get_model_id())
            except:
...
def test():
    ...
    # data reader
    batch_size = args.batch_size
    reader_config = ReaderConfig(f'{global_data_path}/{args.dataset}',
                                 is_test=True)
    reader = reader_config.get_reader()
    ...
    for step_id, data_train in enumerate(test_reader()):
        ...
        if step_id % 10 == 0:
            print("\nBatch %d, Loss %f, Acc %f" %
                  (step_id, avg_loss.avg, avg_accuracy.avg))
        step += 1
