...
class CenterCrop:
    ...
    def __call__(self, img: np.ndarray):
        ...
        crop_top = int(round((img_height - self.crop_size) / 2.))
        crop_left = int(round((img_width - self.crop_size) / 2.))
        return img[crop_top:crop_top + self.crop_size, crop_left:crop_left + self.crop_size, :]
