...
from .tokenizer_util import load_vocab, whitespace_tokenize
...
class CustomTokenizer(object):
    ...
    If you would like tokens, please use `paddlehub.tokenizer.bert_tokenizer.BertTokenizer`.
    """
    ...
    def convert_ids_to_tokens(self, ids, skip_pad_token):
    ...
    def decode(self, token_ids, only_convert_to_tokens=True, skip_pad_token=False, clean_up_tokenization_spaces=True):
        """
    ...
    def clean_up_tokenization(self, out_string: str) -> str:
        ...
        out_string = (out_string.replace(" .", ".").replace(" ?", "?").replace(" !", "!").replace(" ,", ",").replace(
            " ' ", "'").replace(" n't", "n't").replace(" 'm", "'m").replace(" 's", "'s").replace(" 've", "'ve").replace(
                " 're", "'re"))
        return out_string
