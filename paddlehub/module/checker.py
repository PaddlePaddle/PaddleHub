...
class ModuleChecker(object):
    ...
    def _check_module_proto_version(self):
        if self.module_proto_version != module_proto_version:
            logger.warning(
                "Module description file version cannot be aligned with %s version"
                % paddlehub.HUB_SHOW_NAME)
            return False
        return True
    ...
    def _check_hub_version(self):
        if version_compare(self.hub_version, hub_version):
            logger.warning(
                "This Module is generated by the %s with version %s, and the local %s version is %s, which may cause serious incompatible bug. Please upgrade %s to the latest version."
                % (paddlehub.HUB_SHOW_NAME, self.hub_version,
                   paddlehub.HUB_SHOW_NAME, hub_version,
                   paddlehub.HUB_SHOW_NAME))
            return False
        return True
