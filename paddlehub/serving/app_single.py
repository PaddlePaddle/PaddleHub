    ...
    try:
        ...
        for index in range(len(input_img)):
            ...
            with open(os.path.join(output_folder, item), "rb") as fp:
                ...
                b_img = b_head + "," + b_body
                base64_list.append(b_img)
                results[index]["path"] = results[index]["path"].replace(id + "_", "")
                results[index].update({"base64": b_img})
                results_pack.append(results[index])
            os.remove(item)
            os.remove(os.path.join(output_folder, item))
    ...
    try:
        ...
        for index in range(len(input_img)):
            ...
            with open(results[index]["processed"], "rb") as fp:
                ...
                results[index]["origin"] = results[index]["origin"].replace(id + "_", "")
                results[index]["processed"] = results[index]["processed"].replace(id + "_", "")
                results[index].update({"base64": b_img})
                results_pack.append(results[index])
            os.remove(item)
            if output_file_path != "":
...
def create_app(init_flag=False, configs=None):
    ...
    def predict_image(module_name):
        ...
        for key in extra_info.keys():
            if isinstance(extra_info[key], list):
                extra_info[key] = utils.base64s_to_cvmats(eval(extra_info[key][0])["b64s"]) if isinstance(
                    extra_info[key][0], str) and "b64s" in extra_info[key][0] else extra_info[key]
    ...
    def predict_text(module_name):
        ...
        results = predict_nlp(module_info=module_info, input_text=inputs, req_id=req_id, extra=files)
        return results
